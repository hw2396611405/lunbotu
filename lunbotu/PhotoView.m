...
//创建用来缓存图片的文件夹
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"PhotoView"];
    BOOL isDir = NO;
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        //
        //        _scrollView.contentSize = CGSizeMake(3*375, 180);
...
- (instancetype)initWithImageArray:(NSArray *)imageArray {
    return [self initWithImageArray:imageArray imageIndexBlock:nil];
}

+(instancetype)photoViewImageArray:(NSArray *)imageArray  {
    return [self photoViewImageArray:imageArray imageIndexBlock:nil];
...
-(instancetype)initWithImageArray:(NSArray *)imageArray imageIndexBlock:(IndexBlock)imageIndexBlock {
    if (self = [super init]) {
        ...
        self.imageIndexBlock = imageIndexBlock;
    }
    return self;
}

+(instancetype)photoViewImageArray:(NSArray *)imageArray imageIndexBlock:(IndexBlock)imageIndexBlock {
    return [[self alloc]initWithImageArray:imageArray imageIndexBlock:imageIndexBlock];
}
...
#pragma mark 图片点击事件
- (void)imageClick {
    !self.imageIndexBlock?:self.imageIndexBlock(self.currIndex);
}
